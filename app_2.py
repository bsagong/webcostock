    ...
    try:
        ...
        if query.isdigit() and len(query) == 6:
            ...
            try:
                ...
                # 종목명으로 검색
                stock = krx[krx['Name'] == query]
                if len(stock) == 0:
                    print("종목을 찾을 수 없습니다.")
                    return None, -1, None
                code = stock['Code'].iloc[0]
            except Exception as e:
                print(f"종목 검색 중 오류 발생: {e}")
                return None, -1, None
        ...
        try:
            ...
            price = int(df.loc[latest_date, 'Close'])
            
            print(f"조회된 주가 정보: {name}, {price}원 ({latest_date.strftime('%Y-%m-%d')})")
